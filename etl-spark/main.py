#Isso importa o argparse módulo, que fornece uma maneira conveniente de analisar os argumentos da linha de comando.
import argparse
#Isso importa a SparkSession classe do pyspark.sqlmódulo. 
#SparkSession é o ponto de entrada para programação com Spark SQL e API DataFrame.
from pyspark.sql import SparkSession
#Isso importa o functions módulo pyspark.sqle atribui a ele um alias functions. 
#Este módulo fornece várias funções para manipulação e transformação de dados no Spark SQL.
from pyspark.sql import functions as functions 
#Isso importa a regexp_replace função de pyspark.sql.functions. 
#Ele permite que você execute substituições baseadas em expressões regulares em colunas DataFrame.
from pyspark.sql.functions import regexp_replace
#Isso importa a when função de pyspark.sql.functions. Permite expressões condicionais em operações DataFrame.
from pyspark.sql.functions import when
#Isso importa o re módulo, que fornece operações de correspondência de expressão regular.
import re
#Isso importa as funções year, month, to_date e de . 
#Essas funções são comumente usadas para manipulações de data e coluna no Spark SQL.colpyspark.sql.functions
from pyspark.sql.functions import year, month, to_date, col
# Isso importa as classes StringType, IntegerTypee FloatTypede pyspark.sql.types. 
#Essas classes representam tipos de dados que podem ser usados na definição do esquema de um DataFrame.
from pyspark.sql.types import StringType, IntegerType, FloatType
#Essas importações são necessárias para acessar as funcionalidades e tipos de dados necessários em seu script PySpark.
...
if __name__ == '__main__':
    ...
    main(path_input=known_args.path_input,
         ...
         gcs_tmp_bucket="data-pipeline-jp-combustiveis-brasil-pyspark-code"
         )
